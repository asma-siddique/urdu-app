...
class _MatchingQuizScreenState extends State<MatchingQuizScreen> {
  ...
  void _tryMatch() {
    ...
    if (ew == _roundWords[uwi]) {
      ...
      _emotion = AvatarEmotion.happy; _totalScore++;
  ...
  void _nextRound() {
    setState(() {
      _roundsPlayed++;
      if (_roundsPlayed < _totalRounds) { _startRound(); _emotion = AvatarEmotion.excited; }
    });
  }
    ...
    return Directionality(
      ...
      child: Scaffold(
        ...
        body: Column(children: [
          _MatchTopBar(roundsPlayed:_roundsPlayed,totalRounds:_totalRounds,
              totalScore:_totalScore,progress:(_roundsPlayed+1)/_totalRounds),
          Padding(
            ...
            child: Row(children: [
              ...
              Container(
                ...
                child: const Directionality(textDirection: TextDirection.rtl,
                  child: Text('صحیح جوڑ ملائیں!',
                      style: TextStyle(fontFamily:'NotoNastaliqUrdu',fontSize:14,
                          fontWeight:FontWeight.w700,color:Color(0xFF78350F))))),
            ]),
          ),
          Expanded(child: Padding(
            padding: const EdgeInsets.symmetric(horizontal:14),
            child: Row(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
              Expanded(child: Column(children: List.generate(_roundWords.length, (i) {
                final m=_matchedIndices.contains(i),s=_selectedEmoji==i,w=_wrongPair.contains(i);
                return Expanded(child: GestureDetector(
                  onTap: m ? null : () => _onEmojiTap(i),
                  child: AnimatedContainer(duration: const Duration(milliseconds:200),
                    margin: const EdgeInsets.symmetric(vertical:4),
                    decoration: BoxDecoration(color:_cellBg(m,s,w),borderRadius:BorderRadius.circular(14),
                        border:Border.all(color:_cellBorder(m,s,w),width:2),
                        boxShadow:[BoxShadow(color:Colors.black.withOpacity(0.06),blurRadius:4)]),
                    child: Center(child: m
                        ? const Icon(Icons.check_circle_rounded,color:Colors.green,size:26)
                        : Text(_roundWords[i].emoji,style:const TextStyle(fontSize:28))))));
              }))),
              Padding(
                padding: const EdgeInsets.symmetric(horizontal:6,vertical:24),
                child: Column(mainAxisAlignment:MainAxisAlignment.spaceEvenly,
                  children: List.generate(_roundWords.length,
                      (_) => const Text('↔',style: TextStyle(fontSize:14,color:Color(0xFFD1D5DB)))))),
              Expanded(child: Column(children: List.generate(_roundWords.length, (si) {
                final wi=_shuffledUrdu[si],m=_matchedIndices.contains(wi),
                    s=_selectedUrdu==si,w=_wrongPair.contains(-1-si);
                return Expanded(child: GestureDetector(
            ...
            Padding(
              ...
              child: GestureDetector(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(color:_kOrgM,borderRadius:BorderRadius.circular(30),
                      boxShadow:[BoxShadow(color:_kOrgM.withOpacity(0.3),blurRadius:10,offset:const Offset(0,4))]),
                  child: Directionality(textDirection: TextDirection.rtl,
                    child: Text(_roundsPlayed+1>=_totalRounds ? 'نتیجہ دیکھیں  →' : 'اگلا راؤنڈ  →',
          ...
          child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
            Row(mainAxisAlignment: MainAxisAlignment.center,
              children: List.generate(3, (i) => Padding(
                ...
                child: Text(i < stars ? '⭐' : '☆',
                    style: TextStyle(fontSize: i < stars ? 40 : 32,
                        color: i < stars ? Colors.amber : Colors.grey.shade300))))),
            const SizedBox(height:16),
            Container(width:100,height:100,
              decoration: BoxDecoration(shape:BoxShape.circle,color:Colors.white,
                  border:Border.all(color:col,width:4),
                  boxShadow:[BoxShadow(color:col.withOpacity(0.2),blurRadius:16)]),
              child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                Text('$pct%',style:TextStyle(fontSize:28,fontWeight:FontWeight.w900,color:col)),
                const Text('score',style:TextStyle(fontSize:11,color:Colors.grey)),
              ])),
            const SizedBox(height:16),
            Directionality(textDirection:TextDirection.rtl,
              child: Text(excellent ? 'بہت خوب!' : 'اچھی کوشش!',
                  style: TextStyle(fontFamily:'NotoNastaliqUrdu',fontSize:26,
            ...
            GestureDetector(
              onTap: () => setState(() { _roundsPlayed=0; _totalScore=0;
                  _emotion=AvatarEmotion.excited; _startRound(); }),
              child: Container(width:double.infinity,padding:const EdgeInsets.symmetric(vertical:14),
                decoration: BoxDecoration(color:_kPink,borderRadius:BorderRadius.circular(30)),
                child: const Row(mainAxisAlignment:MainAxisAlignment.center,children:[
                  Icon(Icons.refresh_rounded,color:Colors.white,size:20),
                  SizedBox(width:8),
                  Text('Try Again',style:TextStyle(fontSize:15,fontWeight:FontWeight.w800,color:Colors.white))]))),
            ...
            GestureDetector(
              ...
              child: Container(width:double.infinity,padding:const EdgeInsets.symmetric(vertical:14),
                ...
                child: const Row(mainAxisAlignment:MainAxisAlignment.center,children:[
                  ...
                  SizedBox(width:8),
                  Text('Home',style:TextStyle(fontSize:15,fontWeight:FontWeight.w700,color:Color(0xFF6B7280)))]))),
          ]),
        ))),
      ),
    );
...
class _MatchTopBar extends StatelessWidget {
  ...
  const _MatchTopBar({required this.roundsPlayed,required this.totalRounds,
      required this.totalScore,required this.progress});
  @override
  Widget build(BuildContext context) {
